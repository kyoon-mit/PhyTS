...
def _process_files(files, n_per_file, rng, out_ch1, out_ch2, out_params, offset, scaling):
    ...
    for i, path in enumerate(files):
        ...
        with h5py.File(path, 'r') as f:
            ...
            win_indices = rng.choice(n_windows_total, size=n_per_file, replace=False)
...
def main():
    ...
    # -----------------------------------------------------------------------
    train_h5  = sorted(glob.glob(os.path.join(args.data_dir, 'abra_training_*.h5')))
    val_h5    = sorted(glob.glob(os.path.join(args.data_dir, 'abra_validation_*.h5')))
