...
class Project8DataModule(L.LightningDataModule):
    ...
    def __init__(
        ...
        prefetch_factor: Optional[int] = None,   # downstream addition
        persistent_workers: bool = False,         # downstream addition
    ):
        super().__init__()
    ...
    def _loader(self, ds, shuffle):
        ...
        kw = dict(
            ...
            shuffle     = shuffle,
            worker_init_fn = hdf5_worker_init_fn if nw > 0 else None,
        )
        # prefetch_factor / persistent_workers only valid with workers > 0
        if nw > 0:
            if getattr(self.hparams, 'prefetch_factor', None) is not None:
                kw['prefetch_factor'] = self.hparams.prefetch_factor
            if getattr(self.hparams, 'persistent_workers', False):
