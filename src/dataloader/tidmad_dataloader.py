...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python tidmad_dataset.py <path/to/tidmad_output> [window_len]")
        sys.exit(1)

    data_dir   = sys.argv[1]
    window_len = int(sys.argv[2]) if len(sys.argv) > 2 else None
    ...
    print(f"  Files loaded      : {len(ds.paths)}")
    print(f"  Total windows     : {len(ds):,}")
    print(f"  Chunk length (L)  : {ds.chunk_length:,} samples")
    print(f"  Window length     : {ds.window_len:,} samples")
    print(f"  Windows/chunk     : {ds._windows_per_chunk}")
    print(f"  Sample rate       : {ds.sample_rate/1e6:.1f} MHz")
    print(f"  Frequency classes : {ds.n_classes()}")
    print(f"  Class labels (Hz) : {ds.freq_class_labels()}")

    print("\nInspecting first sample (freq_as_index=True) …")
    ch1, ch2, freq_idx = ds[0]
    actual_hz = ds.freq_class_labels()[freq_idx.item()]
    print(f"  ch1  shape={ch1.shape}  mean={ch1.mean():.4f}  std={ch1.std():.4f}")
    print(f"  ch2  shape={ch2.shape}  mean={ch2.mean():.4f}  std={ch2.std():.4f}")
    print(f"  freq_as_index=True  -> freq_idx.item() = {freq_idx.item()}")
    print(f"  This is a CLASS INDEX, not a frequency in Hz.")
    print(f"  Actual frequency    -> ds.freq_class_labels()[{freq_idx.item()}] = {actual_hz} Hz")
    print(f"  Use freq_as_index=True with cross-entropy loss (classification).")
    print(f"  Shared std check: std(ch1)~1.0, std(ch2) reflects SNR")

    print("\nInspecting first sample (freq_as_index=False) …")
    ds_raw = TIDMADDataset(data_dir=data_dir, split="all", return_mode="both",
                           window_len=window_len, normalize=True, freq_as_index=False)
    _, _, freq_hz = ds_raw[0]
    print(f"  freq_as_index=False -> freq_hz.item() = {freq_hz.item()} Hz")
    print(f"  This is the RAW FREQUENCY in Hz as a float32 scalar.")
    print(f"  Use freq_as_index=False with MSE loss (regression).")
    ds_raw.close()

    print("\nRunning one DataLoader batch (num_workers=2) …")
    loader = DataLoader(ds, batch_size=8, shuffle=True,
                        num_workers=2, worker_init_fn=worker_init_fn)
    ch1_b, ch2_b, freq_b = next(iter(loader))
    print(f"  ch1 batch  : {ch1_b.shape}  dtype={ch1_b.dtype}")
    print(f"  ch2 batch  : {ch2_b.shape}  dtype={ch2_b.dtype}")
    print(f"  freq batch : {freq_b}")
    print(f"  NOTE: these are class indices (torch.long), NOT Hz values.")
    print(f"  To recover Hz: [ds.freq_class_labels()[i] for i in freq_b.tolist()]")
    print(f"               = {[ds.freq_class_labels()[i] for i in freq_b.tolist()]}")
