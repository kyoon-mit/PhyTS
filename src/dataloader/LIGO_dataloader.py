...
class LIGODataset(Dataset):
    ...
    def __getitem__(self, idx):
        f = self._get_file()
        seq = f[self.injected_data_key][
            idx, :, self.start_idx : self.end_idx : self.downsample_factor
        ]
        X = torch.as_tensor(seq, dtype=self.dtype)
        if self.clean_data_key is not None:
            clean = f[self.clean_data_key][
                idx, :, self.start_idx : self.end_idx : self.downsample_factor
            ]
...
class LIGODataModule(L.LightningDataModule):
    ...
    def val_dataloader(self):
        return DataLoader(
            self.val_dataset, batch_size=self.val_batch_size, shuffle=False, **self._loader_kwargs
        )

    def test_dataloader(self):
        return DataLoader(
            self.test_dataset, batch_size=self.test_batch_size, shuffle=False, **self._loader_kwargs
        )
