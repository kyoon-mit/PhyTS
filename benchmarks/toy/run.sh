#!/bin/bash
# Toy benchmark pipeline — interactive mode selector.
#
# Usage:
...
SBATCH_EVAL="--partition=shared --nodes=1 --ntasks=1 \
    --cpus-per-task=4 --mem=20G --time=0:10:00 --chdir=$WORKDIR"

EVAL_ARGS="--regressor_raw_ckpt   checkpoints/toy_mlp_regression_raw/best.ckpt \
...
_eval() {
    ...
    sbatch --parsable $SBATCH_EVAL $dep_flag \
        ...
        --wrap="$ACTIVATE && python benchmarks/toy/eval_pipeline.py \
            ...
            $EVAL_ARGS --out_dir plots/toy/${name} --device cpu"
}

submit_evals() {
    local reg_dep=${1:-}  # e.g. "afterok:$JOB_REG_RAW:$JOB_REG_CLEAN"

    # Classical filter: depends only on regressors (no denoiser training job)
    local e
    e=$(_eval classical checkpoints/toy_classical_denoising/best.ckpt \
        configs/toy/train_toy_classical_denoising.yaml "$reg_dep")
    echo "[eval] classical:      job $e"

    # Learned denoisers: dep = denoiser_job:reg_raw_job:reg_clean_job
    _dep() { local d=$1; echo "${d:+$d:}${reg_dep}"; }

    e=$(_eval s4d_mse          checkpoints/toy_s4d_denoising_mse/best.ckpt         configs/toy/train_toy_s4d_denoising_mse.yaml         "$(_dep "${JOB_S4D_MSE:-}")");      echo "[eval] s4d_mse:          job $e"
    e=$(_eval conv_ae_mse      checkpoints/toy_conv_ae_denoising_mse/best.ckpt     configs/toy/train_toy_conv_ae_denoising_mse.yaml      "$(_dep "${JOB_CONV_AE_MSE:-}")");  echo "[eval] conv_ae_mse:      job $e"
    e=$(_eval conv_ae_psd      checkpoints/toy_conv_ae_denoising_psd/best.ckpt     configs/toy/train_toy_conv_ae_denoising_psd.yaml      "$(_dep "${JOB_CONV_AE_PSD:-}")");  echo "[eval] conv_ae_psd:      job $e"
    e=$(_eval conv_attn_ae_mse checkpoints/toy_conv_attn_ae_denoising_mse/best.ckpt configs/toy/train_toy_conv_attn_ae_denoising_mse.yaml "$(_dep "${JOB_CONV_ATTN_MSE:-}")"); echo "[eval] conv_attn_ae_mse: job $e"
    e=$(_eval conv_attn_ae_psd checkpoints/toy_conv_attn_ae_denoising_psd/best.ckpt configs/toy/train_toy_conv_attn_ae_denoising_psd.yaml "$(_dep "${JOB_CONV_ATTN_PSD:-}")"); echo "[eval] conv_attn_ae_psd: job $e"
    e=$(_eval rnn_mse          checkpoints/toy_rnn_denoising_mse/best.ckpt         configs/toy/train_toy_rnn_denoising_mse.yaml          "$(_dep "${JOB_RNN_MSE:-}")");      echo "[eval] rnn_mse:          job $e"
    e=$(_eval rnn_psd          checkpoints/toy_rnn_denoising_psd/best.ckpt         configs/toy/train_toy_rnn_denoising_psd.yaml          "$(_dep "${JOB_RNN_PSD:-}")");      echo "[eval] rnn_psd:          job $e"
    e=$(_eval mlp_denoiser_mse checkpoints/toy_mlp_denoiser_denoising_mse/best.ckpt configs/toy/train_toy_mlp_denoiser_denoising_mse.yaml "$(_dep "${JOB_MLP_MSE:-}")");     echo "[eval] mlp_denoiser_mse: job $e"
    e=$(_eval mlp_denoiser_psd checkpoints/toy_mlp_denoiser_denoising_psd/best.ckpt configs/toy/train_toy_mlp_denoiser_denoising_psd.yaml "$(_dep "${JOB_MLP_PSD:-}")");     echo "[eval] mlp_denoiser_psd: job $e"
}

# ── Interactive menu ───────────────────────────────────────────────────────────

echo ""
echo "Toy benchmark pipeline"
echo "----------------------"
PS3=$'\nSelect: '
select choice in \
