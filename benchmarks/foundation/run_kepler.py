"""Zero-shot classification on Kepler Q9 v3 via FM linear probe.

Usage
-----
    python benchmarks/foundation/run_kepler.py \
        --zip data/keplerq9v3.zip \
        --models timemoe granite_ttm \
        --out_dir plots/kepler_q9v3/linear_probe
"""
...
# Default temporal pooling per model, informed by architecture:
#   encoders (bidirectional) → mean; decoders (causal) → last-token.
_DEFAULT_POOL = {
    "moment":      "mean",   # bidirectional encoder
    "chronos":     "mean",   # T5 encoder output
    "moirai":      "mean",   # bidirectional encoder
    "granite_ttm": "mean",   # mixer (no attention)
    "timemoe":     "last",   # causal decoder
    "timesfm":     "last",   # causal decoder
    "lagllama":    "last",   # causal decoder
}
...
def main():
    ...
    p.add_argument("--out_dir", default="plots/kepler_q9v3/linear_probe")
    ...
    p.add_argument("--C", type=float, default=1.0, help="Logistic regression inverse regularisation")
    ...
    for model_name in args.models:
        ...
        # MOIRAI embed needs a fixed patch size (auto-search triggers multiple
        # encoder passes and makes the hook capture ambiguous).
        if model_name.lower() == "moirai":
            load_kwargs["patch_size"] = 32
        # Lag-Llama wrapper needs the local checkpoint path.
