#!/usr/bin/env bash
# Direct-run script for brigand (16 GiB).
# Cells: Project 8 LoRA for chronos, moirai, timemoe, timesfm — sequential.
set -u
...
P8_LORA_COMMON=(
  ...
  --adapter lora
  --epochs 10 --head_lr 1e-3 --backbone_lr 1e-4
  --lora_r 8 --lora_alpha 16 --patience 3
)

for m in chronos moirai timemoe timesfm; do
  run_cell "p8_lora_${m}" benchmarks/foundation/run_project8_finetune.py \
    --models "${m}" "${P8_LORA_COMMON[@]}"
done
